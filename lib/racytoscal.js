...
function fromRascal(s){
	   ...
	   if (t.table!=null) {
		   var attach = document.getElementById(t.table.id);
		   var table = document.createElement("table");  
		   table.setAttribute("id", t.table.tableId);
		   var width = parseInt(t.table.width);
		   var height = parseInt(t.table.height);
		   for (var i=0;i<height;i++) {
			  var row = document.createElement("tr");
			  for (var j=0;j<width;j++) {
				  var td = document.createElement("td");
				  td.setAttribute("id", t.table.cellId+"_"+i+"_"+j);
				  td.setAttribute("class", t.table.cellId);
				  row.appendChild(td);
		      }
	  ...
	  if (t.onclick!=null) {
		  var el = document.getElementById(t.onclick); 
