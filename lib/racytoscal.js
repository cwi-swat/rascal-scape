...
function httpGet(callback, key, sync)
{
    ...
    xmlHttp.open("GET", currentURL+(currentURL.endsWith("/")?"":"/")+key, sync); // true for asynchronous 
    xmlHttp.send(null);
}
...
function addHandlerClick(event, widget, sync, callback) {
	 // alert(widget);
	  var el = findEl(widget);
	  for (var i=0;i<el.length;i++)
	    el[i]["on"+event]=function(evt){
			 var ele = evt.target;
			 httpGet(callback
			  , event+"/"+ele.id, sync);
		   };
...
function findEl(name) {
   ...
   if (el!=null && el.length>0) return el;
